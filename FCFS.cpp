...
int main()
{
  int n;
  process p[100];
  //int ct[100],tat[100],st[100],wt[100];
  float tatavg,wtavg;
  ...
  {
    p[i].pid=i+1;
    cout<<"\nEnter at for p"<<p[i].pid<<" : ";
    cin>>p[i].at;

    cout<<"\nEnter bt for p"<<p[i].pid<<" : ";
    cin>>p[i].bt;
    cout<<endl;
  }
  ...
  sort(p,p+n,compareId);
  //cout<<"\nwt = "<<total_wt<<endl;;
  tatavg=(float)total_tat/n;
  wtavg=(float)total_wt/n;
  ...
  for(int i=0;i<n;i++)
  {
    cout<<p[i].pid<<"\t"<<p[i].at<<"\t"<<p[i].bt<<"\t"<<p[i].ct<<"\t"<<p[i].wt<<"\t"<<p[i].tat<<endl;
  }

  cout<<endl;
  cout<<"avg tat : "<<tatavg<<endl;
  cout<<"avg wt  : "<<wtavg<<endl;
  return 0;
}
*/
